...
> Последнее обновление: 2026-07-09 (импорт проекта в Replit; актуализация документации: 58 модулей (`test_imports.py`), 894 сценария (`test_scenarios.py`), `database.py` ~20 160 строк, 41 web-роут, 108 Jinja2-шаблонов, 34 фоновые задачи APScheduler) · 58 модулей · GitHub `28099f7`
...
## 10. ВЕБ-СЛОЙ: ФАЙЛЫ, РЕШЕНИЯ, ФИЧИ (перенесено из replit.md)

> Этот раздел — детальный справочник веб-слоя. Краткая версия — в `replit.md`.

### 10.1 Где что живёт — Web Interface (`web/`)
...
### 10.2 Полный каталог архитектурных решений
...
- **Messenger-feel upgrade (чат, фазированно)**: реакции-эмодзи (`message_reactions`/`dm_reactions`, toggle, чипы + bulk-load) · ответы/цитаты (`reply_to_id`, кликабельная цитата → скролл к оригиналу) · упоминания `@имя` (резолв в участников, адресный web push + подсветка, автокомплит) · правка автором (`edited_at`, метка «изм.») · **live-доставка тем по WS** (`/ws/topic` + `TopicConnectionManager`: «поке» `{type:'new'}` из async `chat_send` → клиент делает мгновенный per-user `poll()`; poll = 4-сек фоллбэк; sync edit/delete/react остаются на poll) · двойные галочки ✓/✓✓ в ЛС (из `is_read`, realtime-флип по WS-квитанции) · **пересылка** (`forwarded_from TEXT`, POST `/chat/forward`: источник/назначение группа↔ЛС; вложения физически копируются `shutil.copy2` в uploads назначения — удаление оригинала не ломает; `forwarded_from` хранит исходного автора через всю цепочку; DM-источник гейтится участием; «↪ Переслано от X» в обоих типах пузырей) · **markdown-форматирование** (рендер-тайм в общем `formatMsg`: `**жирный**`/`*курсив*`/`_курсив_`/`` `код` ``/```` ```блок``` ````/`~~зачёрк~~`/авто-ссылки http(s); esc() ПЕРВЫМ → код прячется в nonce-плейсхолдеры → markdown → restore; `javascript:` не линкуется; текст хранится сырым, бэкенд не трогается; правка показывает сырой текст) · **мультифайл во вложениях ЛС** (`dm_message_files`, до 10 файлов; пузырь ЛС рендерит `x-for` по `m.files` с legacy single-file fallback — зеркало группового; WS-обработчик входящих DM переносит `data.files`, иначе мультифайл по WS деградировал бы до первого) · **превью ссылок (OpenGraph)** (`web/link_preview.py`: SSRF-safe async-фетч — http/https only, валидация host/IP на каждом редиректе против private/loopback/link-local/reserved, ≤3 ручных редиректа `allow_redirects=False`, 512KB body cap, 8с таймаут; regex-парсинг OG/twitter/title — bs4 нет; self-contained кэш `data/link_cache.db` OK_TTL=7д/FAIL_TTL=1ч; GET `/api/chat/link-preview` — auth + chat-гейт + rate-limit 40/60с по telegram_id; фронт: `loadLinkPreview` на `x-init` текста, карточка в обоих пузырях, ключ `'g'+id`/`'d'+id`, инвалидация ключа при правке). Все фичи зеркалятся: группы (chat_messages, poll/poke) ↔ ЛС (direct_messages, WS). Детали — `.agents/memory/chat-*.md`
...
- **assetlinks.json**: `GET /.well-known/assetlinks.json` route in `web/app.py` — Digital Asset Links для Android TWA; содержит `package_name=com.dailysales.app` + SHA-256 fingerprint keystore; Android проверяет этот файл при установке/запуске TWA APK, чтобы убедиться что APK доверяет домену; `Cache-Control: no-cache`; fingerprint: `25:E3:EB:BB:2B:72:C7:12:CB:15:59:AD:1C:E9:6B:20:8A:4E:EB:19:97:B9:93:8F:37:47:31:96:82:BB:A1:1D`
...
### 10.3 Полный список фич (Product)
...
- **Android APK (TWA)**: `twa-manifest.json` — TWA конфиг (`packageId=com.dailysales.app`, fingerprint SHA-256); `android/` — Android Gradle проект для сборки TWA (settings.gradle, app/build.gradle, AndroidManifest.xml, gradle.properties); `.github/workflows/build-twa.yml` — GitHub Actions прямой Gradle build (Java 17 + Android SDK + `gradle :app:assembleRelease` → GitHub Releases); signing через env vars `KEYSTORE_PATH`/`KEYSTORE_PASSWORD`; `.well-known/assetlinks.json` на сервере; GitHub Secrets: `KEYSTORE_BASE64` + `KEYSTORE_PASSWORD`; APK устанавливается как нативное приложение Android. **⚠️ Gotcha**: bubblewrap CLI НЕ работает в GitHub Actions (25+ попыток, «androidSdk isn't correct») → прямой Gradle build; `android/gradle.properties` с `android.useAndroidX=true` ОБЯЗАТЕЛЕН. **Точки скачивания**: `GET /download/android` (302 → `github.com/.../releases/latest`); кнопка на лендинге, баннер на дашборде (localStorage dismiss), карточка в настройках, callback `apk_info` в боте (профиль). **Webhook**: `POST /webhook/apk-release` → скачивает APK локально + рассылает уведомление всем owner_id орг. **Суперадмин**: `GET /admin/apk` — страница со статистикой скачиваний (Chart.js), историей релизов, источниками переходов
- Reports filters persistence: фильтры период/категория/продавец сохраняются при смене периода, магазина, группировки, кастомных дат (`web/templates/reports/index.html`, 5 мест)
