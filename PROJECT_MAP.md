...
> Последнее обновление: 2026-06-14 (сессия 657: TWA Android APK pipeline, фиксы reports/start.py/scheduled_notifications) · 55 модулей · GitHub актуально · Amvera актуально
...
- **Android APK (TWA)**: `twa-manifest.json` — Bubblewrap конфиг (`packageId=com.dailysales.app`); `.github/workflows/build-twa.yml` — GitHub Actions сборка APK при каждом push (Java 17 + Android SDK + Bubblewrap → `bubblewrap build --skipPwaValidation` → GitHub Releases); `.well-known/assetlinks.json` уже живёт на сервере; GitHub Secrets: `KEYSTORE_BASE64` + `KEYSTORE_PASSWORD`; APK устанавливается как нативное приложение Android поверх PWA. **Точки скачивания**: `GET /download/android` (302 → `github.com/.../releases/latest`); кнопка на лендинге, баннер на дашборде (localStorage dismiss), карточка в настройках, callback `apk_info` в боте (профиль). **⚠️ Gotcha сборки**: bubblewrap требует `cmdline-tools/latest/` — GitHub Actions runner хранит как `cmdline-tools/13.0/`; workflow создаёт symlink автоматически
