...
### v13 — 2026-06-30: Мобильный UX /tasks + Канбан адаптив (Tasks #13–#18)

**`/tasks` — адаптив под мобильный экран (`web/templates/tasks/index.html`):**
- **Дропдаун «Виды»** (список/Канбан/Гант) — `class="lg:hidden"`: виден только на мобильном, на десктопе скрыт (там вкладки в шапке)
- **Коллапс фильтров** — кнопка «⚙ Фильтры» + `:class="filtersOpen ? 'block' : 'hidden lg:block'"` + `x-cloak`; на десктопе всегда раскрыты
- **Lazy quick-add** — форма «+ Задача» `:class="qaOpen ? 'flex' : 'hidden lg:flex'"` + `@submit="qaOpen=false"`; не рендерится до первого тапа
- **Горизонтальный скролл** строк статуса и сортировки — `overflow-x-auto` + `flex-nowrap`
- **Синхронизация активных кнопок сортировки/статуса** после HTMX-обновления: функция `_syncTaskCtl(url)` в `htmx:afterSwap` читает `data-ctl`/`data-val`/`data-on`/`data-off` на кнопках и синхронизирует по `e.detail.xhr.responseURL`

**Канбан (`web/templates/tasks/kanban.html`):**
- Горизонтальный скролл доски — `overflow-x-auto` + фиксированная ширина колонок `min-w-[260px]`
- Фикс кнопки «зависшей» current-column: `relocateCard()` скрывает кнопку текущей колонки сразу после перемещения
- Возврат карточки без перезагрузки при drag-отмене — клиентский `relocateCard()` обратно

**Диаграмма Гант (`web/templates/tasks/gantt.html`):**
- Горизонтальный скролл таймлайна; колонка имени задачи фиксирована слева

**Регресс-тесты Канбана (`test_web_smoke.py`):**
- `check_kanban_counts_wip` — после каждого `relocateCard()` проверяет счётчики source/dest, `bg-red-100` WIP-badge (появляется при переполнении, исчезает при освобождении), «Нет задач» плейсхолдер при опустошении
- `check_kanban_buttons_nonadmin` — не-администратор видит 4 кнопки (нет «Отменить»); current-column кнопка скрыта
- Фикс `check_kanban_quick_move`: селектор `:visible` вместо `.first` (hidden current-column кнопка шла первой)

---

### v12 — 2026-06-26: Аудит безопасности модуля задач

**Изменения поведения маршрутов `web/routes/tasks.py`:**
- `POST /tasks/{id}/kanban` — `cancel` теперь требует `is_assignee` или `is_creator`; иначе 403
- `POST /tasks/assign` (`self_assign`) — валидация: задача должна быть `assign_all` или `shop`-типа и не иметь исполнителя; иначе 400
- `POST /tasks/bulk` — каждая задача в цикле проверяется SELECT перед изменением статуса/удалением; несуществующие пропускаются
- `POST /tasks/topics/{id}/edit` / `DELETE /tasks/topics/{id}` — SELECT из `task_topics` перед мутацией; 404 если не найдено
- `POST /tasks/{id}/log-time` — добавлен `can_act` guard (аналогично другим действиям над задачей)
- `DELETE /tasks/time-log/{id}` — SELECT задачи перед удалением лога; 404 если задача не найдена
- `/tasks/my/complete` (бот + веб) — отфильтровывает задачи чужих магазинов через `shop_membership`
- `POST /tasks/{id}/watch` / `unwatch` — проверка принадлежности к org перед подпиской; 403 если нет

**Email-only пользователи**: `get_tasks()` теперь берёт `org_db` из `web_credentials` для синт. `tg_id < 0` — ранее использовался глобальный контекст, что давало пустой список или чужие задачи.

**Итог**: 27/27 POST-роутов в tasks.py покрыты Auth+CSRF.
...
- Новый раздел веб-кабинета **`/org-structure`** (owner и super_admin) — 3 вкладки:
  ...
  - **Кастомные роли** — `org_roles` с набором прав (`can_*`) и списком доступных модулей; только при оплаченном модуле `org_structure` (349₽)
  - **Инфо** — справка по уровням доступа (minimal/full)
- **Карточка сотрудника** (Персонал) — назначение кастомной роли + подразделения + персональный доступ к модулям (allow/deny)
- Гейт уровня: `org_structure_level(tg)` → `full` если `has_module(owner_tg,'org_structure')`, иначе `minimal`
- nav: `user_module_access` (deny) скрывает модуль у конкретного сотрудника поверх биллинга
- Кастомная роль раскладывается в стандартные поля `user_org_mapping` → весь старый authz без изменений; scope сети = `network`
