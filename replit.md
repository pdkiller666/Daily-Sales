...
Sales/inventory/daily reports · multi-org with invite codes & roles · sales plans + milestone alerts · dashboards · contests · work schedules + shift templates · salary transparency · subscriptions/payments (SBP + YooKassa) + modular billing · rankings · Excel import/export · Google Sheets integration · internal org chat (topics + DM; реакции/ответы/упоминания @/правка/закреп (группы)/двойные галочки ✓✓ (ЛС)/пересылка/markdown/превью ссылок/live-WS) · push & browser notifications · email/password login · flexible org structure.
...
0b. **`products` row positional indices**: id[0] name[1] category[2] price[3] created_at[4] photo_file_id[5] description[6] **article**[7] **barcode**[8] **old_price**[9] — barcode добавлен 2026-06-15, old_price (зачёркнутая цена для акций) добавлен 2026-06-19; все доп. поля через `ALTER TABLE` (CREATE TABLE содержит только id..created_at); срезы `row[:7]`/`row[:8]` не включают old_price — проверять при новых фичах.
...
26. **Веб-чат rate-limit за прокси — по `telegram_id`, НЕ по `request.client.host`** — за reverse-proxy (Amvera) IP общий для всех → лимит по IP блокирует ВСЕХ. Все rate-helpers чата ключуются по `telegram_id`. Курсор live-удалений `get_chat_deleted_ids_since` использует `deleted_at >= since_ts` (строгий `>` теряет удаления в ту же секунду; клиентский filter идемпотентен). Прочитанное — серверная таблица `chat_read_state` (upsert через MAX), НЕ localStorage. DM-удаление: `dm_delete` async + WS `{"type":"delete"}` обоим (`row[1]`,`row[2]`); вложения в `dm_send` — реальный att_id (`_load_dm_files_bulk` ДО payload), id сообщения ≠ id вложения.
