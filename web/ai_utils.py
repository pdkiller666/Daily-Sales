...
ФОРМАТ вызова инструмента — ТОЛЬКО одна строка, ничего кроме:
  TOOL_CALL: {{"tool": "название_инструмента", "params": {{"ключ": "значение"}}}}

КОГДА вызывать инструменты:
• ПРОДАЖИ за любой прошлый или заданный период → get_sales_period(date_from, date_to)
  ! В контексте есть ТОЛЬКО «сегодня» и «текущий месяц» (то, что уже в промпте).
  ! Всё остальное — вчера, прошлая неделя, любые конкретные даты — ТРЕБУЕТ инструмента.
  ! НИКОГДА не говори «у меня нет данных» о продажах до вызова этого инструмента.
• ТРЕНД по дням → get_daily_sales(date_from, date_to) — лучший/худший день, динамика
• СРАВНЕНИЕ периодов → get_sales_comparison() — эта неделя vs прошлая, рост/падение
• КАТЕГОРИИ товаров → get_category_breakdown(date_from, date_to) — структура продаж
• КОНКРЕТНЫЙ продавец → get_seller_stats(seller, date_from, date_to) — личные показатели
• ПЛАНЫ детально → get_plans_detail() — разбивка по магазинам с прогресс-барами
• ОСТАТКИ → get_inventory() / get_low_stock()
• РЕЙТИНГ → get_rankings(type=sellers/shops/cities, date_from, date_to)
• ЗАРПЛАТЫ → get_salary_summary(year, month)
• ОТСУТСТВИЯ → get_absences(year, month)
• ЗАДАЧИ → get_tasks(status)
• ГРАФИК → get_work_schedule(year, month)
• ЧЕКИ → get_recent_transactions() / get_transaction_by_id()

ПОДСЧЁТ ДАТ (используй даты из промпта):
• «вчера» = сегодня − 1 день
• «эта неделя» = от ближайшего пн до сегодня
• «прошлая неделя» = пн − 7 дней до вс − 1 день
• «прошлый месяц» = первый и последний день предыдущего месяца
• Всегда передавай date_from/date_to в формате ГГГГ-ММ-ДД

ПЕРИОД для month/year инструментов (зарплата, отсутствия, график):
• Если пользователь НЕ назвал месяц/год → НЕ передавай эти параметры (инструмент сам берёт текущий).
• Называй period только когда пользователь сам сказал «за апрель», «в мае» и т.п.

МНОГОШАГОВЫЕ ЗАПРОСЫ: Для сложного анализа вызывай инструменты последовательно.
Например: «сравни эту неделю с прошлой и скажи какие категории тянут вниз» →
1. get_sales_comparison() 2. get_category_breakdown(прошлая неделя) 3. финальный вывод.

ЗАПРЕЩЕНО: не придумывай числа. Если инструмент вернул данные — используй их.
ФОРМАТ ответа: 3–5 предложений, по-русски, без markdown и заголовков."""
...
def build_product_search_prompt(query: str, products_text: str) -> str:
    """Промпт для семантического поиска товаров по запросу на естественном языке.

    products_text: строка с каталогом в формате "id|name|category|price\\n..."
    Возвращает промпт, LLM должен ответить JSON: {"ids": [1, 3, 5], "note": "..."}
    """
    return (
        f"Каталог товаров магазина (id|название|категория|цена):\n{products_text}\n\n"
        f"Запрос покупателя: «{query}»\n\n"
        "Найди товары из каталога выше, которые наиболее точно соответствуют запросу.\n"
        "Учитывай смысл запроса: синонимы, ценовые ограничения, категории.\n"
        "Верни ТОЛЬКО валидный JSON без markdown и пояснений:\n"
        '{"ids": [<список id подходящих товаров, макс. 10>], '
        '"note": "<одно предложение — почему выбраны эти товары>"}\n'
        "Если подходящих товаров нет — верни {\"ids\": [], \"note\": \"Ничего не найдено\"}.\n"
        "Никаких пояснений до или после JSON."
    )


def build_cross_sell_prompt(pairs: list) -> str:
    """Промпт для анализа совместных покупок (cross-sell / up-sell).

    pairs: список [(name_a, name_b, freq), ...]  — пары товаров + частота совместных покупок.
    """
    if not pairs:
        return (
            "В данном магазине пока недостаточно данных о совместных покупках.\n\n"
            "Дай 2–3 общих совета по организации перекрёстных продаж в розничном магазине. "
            "Тон практический, без воды. Опирайся на лучшие практики ритейла."
        )
    pair_lines = "\n".join(
        f"  {i+1}. {a} + {b} — куплено вместе {n} раз"
        for i, (a, b, n) in enumerate(pairs[:12])
    )
    return (
        f"Данные о совместных покупках в магазине за последние 60 дней:\n{pair_lines}\n\n"
        "На основе этих данных напиши практический анализ (3–4 предложения):\n"
        "1) Выяви 1–2 самые сильные пары — назови их и объясни, почему их выгодно продавать вместе.\n"
        "2) Дай конкретный совет по размещению или бандлированию этих пар на точке продаж.\n"
        "3) Назови одну пару, которую стоит использовать для up-sell скрипта продавца.\n"
        "Опирайся только на данные выше. Тон деловой, конкретный, без общих фраз. "
        "Без markdown и заголовков."
    )


def build_price_advice_prompt(
    product_name: str,
    category: str,
    current_price: float,
    ...
    """Промпт для AI-рекомендации по цене товара.

    sales_30/sales_90: {"total_qty": int, "total_rev": float, "days_with_sales": int,
                        "avg_per_day": float, "transactions": int}
    Возвращает промпт; LLM должен ответить: action ("raise"/"lower"/"keep") + rationale.
    """
    old_str = f" (старая цена: {int(old_price):,} ₽)" if old_price and old_price > current_price else ""
    s30 = f"{sales_30.get('total_qty', 0)} шт. за {sales_30.get('days_with_sales', 0)} дн. с продажами (ср. {sales_30.get('avg_per_day', 0):.1f} шт./день)"
    s90 = f"{sales_90.get('total_qty', 0)} шт. за {sales_90.get('days_with_sales', 0)} дн. с продажами (ср. {sales_90.get('avg_per_day', 0):.1f} шт./день)"

    trend = ""
    avg30 = sales_30.get("avg_per_day", 0)
    avg90 = sales_90.get("avg_per_day", 0)
    if avg90 > 0 and avg30 > 0:
        chg = (avg30 - avg90) / avg90 * 100
        direction = "растёт" if chg >= 5 else ("падает" if chg <= -5 else "стабильный")
        trend = f" Тренд спроса: {direction} ({chg:+.0f}% к 90-дн. норме)."
    elif avg30 == 0:
        trend = " В последние 30 дней продаж не было."

    return (
        f"Товар: «{product_name}»\n"
        f"Категория: {category}\n"
        f"Текущая цена: {int(current_price):,} ₽{old_str}\n"
        f"Продажи за 30 дней: {s30}\n"
        f"Продажи за 90 дней: {s90}\n"
        f"{trend}\n\n"
        "Дай рекомендацию по цене (1–2 предложения):\n"
        "1) Вынеси вердикт: поднять / снизить / оставить — и назови конкретную цену или диапазон.\n"
        "2) Обоснуй на основе скорости продаж и тренда из данных выше.\n"
        "Опирайся только на предоставленные данные. Тон деловой, конкретный. Без markdown."
    )
