...
def _get_user_ai_history(tg_id: int, days: int, feature: str | None) -> list:
    ...
    for r in rows:
        r["feature_label"] = _FEATURE_LABELS.get(r["feature"], r["feature"])
    return rows


@router.get("/ai/history")
def ai_history_page(request: Request):
    """Личный журнал AI-запросов пользователя."""
    from web.auth import get_session_user
    user = get_session_user(request)
    if not user:
    ...
    try:
        entries = _get_user_ai_history(tg_id, days, feature or None)
    except Exception as exc:
