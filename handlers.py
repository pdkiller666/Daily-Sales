...
async def apk_info_handler(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        "📱 <b>Приложение DailySales (Android)</b>\n\n"
        "Это <b>TWA-приложение</b> — оболочка вокруг веб-кабинета.\n\n"
        "✅ <b>Обновления автоматические</b> — все новые функции и исправления "
        "применяются при каждом открытии приложения, без переустановки APK.\n\n"
        "📲 <b>APK скачивается один раз</b> при первой установке.\n"
        "После этого обновлять приложение не нужно — интерфейс всегда актуален.\n\n"
        "💡 Работает без Google Play — установка из файла."
    )
    builder = InlineKeyboardBuilder()
...
_HELP_TEXTS: dict[str, str] = {
    ...
    "sheets": (
        ...
        "<b>Доступно при подключённом модуле «Интеграции»</b> — в любом тарифе.\n\n"
    ...
    "subscription": (
        ...
        "• <b>Тариф = объём</b> (товары, магазины, продажи в месяц)\n"
        "• <b>Модули = возможности</b> (аналитика, команда, уведомления, "
        "интеграции, чат…) — подключаются отдельно в любом тарифе\n"
        "Актуальные условия и цены — в разделе «💳 Подписка».\n\n"
        "<b>🎁 Пробный период:</b>\n"
        "14 дней бесплатно — временный полный доступ ко всем модулям и "
        "безлимитному объёму.\n\n"
        "<b>Способы оплаты:</b>\n"
    ...
    try:
        ...
        for p in plans:
            ...
            lines.append(
                ...
                f"  {prod_str} товаров · {shop_str} маг. · {sales_str} продаж/мес"
            )

        text = "\n".join(lines)
        text += (
            "\n\n<b>🧩 Возможности</b> (аналитика, команда, уведомления, "
            "интеграции, чат и др.) подключаются отдельными модулями в любом тарифе — "
            "тариф определяет только объём."
        )
        text += (
            "\n\n<b>🎁 Пробный период:</b>\n"
            "14 дней бесплатно — временный полный доступ ко всем модулям и "
            "безлимитному объёму. После окончания останется выбранный тариф (объём) "
            "и подключённые модули.\n\n"
            "<b>Способы оплаты:</b>\n"
            "• <b>СБП</b>: переведите сумму → загрузите скриншот чека → "
